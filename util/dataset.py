...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None, filter_intersection=False):
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        #print("===========Problem: ", os.path.join(data_root, line_split[0]))
        image_name = os.path.normpath(os.path.join(data_root, line_split[0]))
        label_name = os.path.normpath(os.path.join(data_root, line_split[1]))
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
        ...
        self.base_path = os.path.normpath(os.path.join(self.base_data_root, mode, str(self.split)))
        ...
        print("fss_data_list_path", fss_data_list_path)
        print("fss_sub_class_file_list_path",fss_sub_class_file_list_path)
        # Write FSS Data
        ...
        #print("======Problem:", self.sub_class_file_list)
    ...
    def __getitem__(self, index):
        ...
        if 'VOC' in image_path:
            ...
            query_name = tmp_sperate[3]
        #print("query_name", query_name, "\n")
        image = cv2.imread(image_path, cv2.IMREAD_COLOR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.float32(image)
        if image is None:
            raise ValueError(f"---Failed to read image at {image_path}, label path: {label_path}---")

        img_cv2 = image.copy()
        #print("label_path: ",os.path.normpath(label_path))
        label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE)
        #print("base:", os.path.normpath(os.path.join(self.base_path, label_path.split('/')[-1])))
        ...
        #print(r"==========================Problem: ", self.sub_class_file_list[class_chosen], "\n \n")
        ...
        for k in range(self.shot):
            ...
            #print("support_image_path", support_image_path)
            while ((
                           support_image_path == image_path and support_label_path == label_path) or support_idx in support_idx_list):
                support_idx = random.randint(1, num_file) - 1
                support_image_path, support_label_path = file_class_chosen[support_idx]
                #print("===========support_image_path", file_class_chosen, "\n")
                # Normalize the paths to avoid escape character issues
                #support_image_path = os.path.normpath(support_image_path)
                #support_label_path = os.path.normpath(support_label_path)
        ...
        for k in range(self.shot):
            ...
            if support_image is None:
                raise ValueError(f"---Failed to read support_image_path at {support_image_path}, support_image_path: {support_image_path}---")
            #print("issue 1: \n\n")
        ...
        #print("issue 2:", raw_label, " \n\n")
        raw_label_b = label_b.copy()
        support_image_list = [[] for _ in range(self.shot)]
        support_label_list = [[] for _ in range(self.shot)]
        if self.transform is not None:
            image, label, label_b, img_cv2 = self.transform_tri(image, label, label_b, img_cv2)  # transform the triple
            for k in range(self.shot):
        ...
        if self.mode == 'train':
            ...
            return image, query_name, label, label_b, s_x, s_y, subcls_list, class_chosen-1, raw_label, raw_label_b, img_cv2
        elif self.mode == 'demo':
