...
from util.visu import make_episode_visualization
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='F3SS')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/coco/coco_split0_resnet50_manet.yaml', help='config file') # coco/coco_split0_resnet50.yaml
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, ori_label, ori_label_b, img_cv2) in enumerate(val_loader):
            ...
            output, meta_out, base_out = model(s_x=s_input, x_cv2=img_cv2, que_name=input_name, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                               cat_idx=subcls, class_name=class_name)
            model_time.update(time.time() - start_time)

            target_vis = target
            target_b_vis = target_b
            ...
            output_pred = map_to_square(F.interpolate(output, size=(H, W), mode='bilinear', align_corners=True))
            ...
            loss = criterion(output_pred, target)
            final_out_interp = F.interpolate(output, size=(473, 473), mode='bilinear', align_corners=True)

            output = output_pred.max(1)[1]
            meta_out = meta_out.max(1)[1]
            base_out = base_out.max(1)[1]
            # -----------------------Overlay-------------------
            for i in range(len(input)):

                # Extract and convert inputs to NumPy
                support_imgs_np = s_input[i].cpu().numpy()  # shape: (K, 3, H, W)
                query_img_np = input[i].cpu().numpy()  # shape: (3, H, W)
                support_labels_np = s_mask[i].cpu().numpy()  # shape: (K, H, W)
                query_gt_np = target_vis[i].cpu().numpy()  # shape: (H, W)

                # Convert prediction to numpy (still 4D: B x C x H x W)
                preds_np = final_out_interp[i].unsqueeze(0).detach().cpu().numpy()  # shape: (1, C, H, W)
                img_name = input_name[i].split('.')[0]
                # Ensure query image is in H x W x 3 for visualization
                query_img_np = np.transpose(query_img_np, (1, 2, 0))  # (H, W, 3)
                #if (img_name == "COCO_val2014_000000105923" or
                #        img_name == "COCO_val2014_000000278705" or img_name == "COCO_val2014_000000099810"):
                base_name = img_name + str(e)
                # Visualize and save
                make_episode_visualization(
                    img_s=support_imgs_np,
                    img_q=query_img_np,
                    gt_s=support_labels_np,
                    gt_q=query_gt_np,
                    preds=preds_np,
                    save_path=base_name
                )
                print("Image saved", base_name)
            #-----------------------Overlay-------------------
